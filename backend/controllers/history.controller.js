...
export const addToHistory = async (req, res) => {
  const { userId } = req;
  const { songId, titulo, artista, url, portada, identifier, isIa, originalUrl } = req.body; // 'songId' local OR 'identifier' for IA

  if (!songId && !identifier) {
    return res.status(400).json({ error: 'Falta el ID de la canción (songId) o identifier.' });
  }

  try {
    // -----------------------------------------------------------------
    // LÓGICA DE "ENRUTAMIENTO"
    // 1. Verificamos si la canción existe en nuestra tabla 'canciones' (local)
    ...
    if (song) {
      ...
      // -----------------------------------------------------------------
      // CASO 2: No es local. Puede ser IA (si viene 'isIa' o 'identifier')
      if (isIa || identifier) {
        const rawId = identifier || songId;
        const iaIdentifier = String(rawId).split('/')[0]; // Normalizar: quitar filename si viene
        const iaUrl = originalUrl || url || null; // Prefer originalUrl si está disponible
        ...
        `;
        params = [userId, iaIdentifier, titulo, artista, iaUrl, portada];
      } else {
        // No es local ni tiene identifier → fallback: usar songId como identificador de IA
        query = `
          INSERT INTO ia_history (user_id, ia_identifier, titulo, artista, url, portada, played_at, play_count)
          VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, 1)
          ON CONFLICT(user_id, ia_identifier) DO UPDATE SET
