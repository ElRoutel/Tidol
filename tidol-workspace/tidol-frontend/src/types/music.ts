 ...
 */
export type SourceType = 'local' | 'youtube' | 'spotify' | 'soundcloud' | 'torrent' | 'internet-archive' | 'internet_archive' | 'musicbrainz' | 'radio';

/** Plataforma legal de reproducción (alineada con el enum Platform del backend Rust). */
export type Platform = 'youtube' | 'spotify' | 'soundcloud' | 'internet_archive';
...
export interface UnifiedTrack {
    ...
    /** Plataforma legal de origen para reproducción por embed (YouTube/Spotify/SoundCloud/IA). */
    platform?: Platform;
    /** ID de vídeo de YouTube si la pista ya está resuelta a YouTube. */
    videoId?: string;
    /** URL de embed (iframe) de la plataforma de origen. */
    embedUrl?: string;
    /** URL para abrir la pista en la web/app de la plataforma. */
    externalUrl?: string;
