...
export class TidolAudioEngine extends EventTarget {
    ...
    private async resolveStreamUrl(track: UnifiedTrack): Promise<{
        ...
        // El motor nativo solo reproduce audio DIRECTO y LEGAL ya resuelto:
        // stream CC de Internet Archive o ficheros locales del usuario. La URL
        // se inyecta por PlayerContext.startPlayback tras resolvePlayback().
        // (El antiguo endpoint /api/v1/stream/ — proxy de audio con copyright —
        //  fue eliminado del backend por la migración legal.)
        if (track.playbackUrl) {
            return {
                streamUrl: track.playbackUrl,
                provider: track.sourceType || 'direct',
                metadata: { source: track.sourceType },
                fallbacks: []
            };
        }

        throw new Error('[AudioEngine] No hay URL de audio directo legal para esta pista');
    }
