...
export default function InternetArchivePage() {
  ...
  useEffect(() => {
    const fetchAlbumData = async () => {
      try {
        ...
        const normalizedTracks = normalizeTrackList(audioFiles, 'internet-archive');

        setAlbum(albumInfo);
        if (normalizedTracks.length === 0) setError('No se encontraron archivos de audio.');
        setAllFiles(normalizedTracks);
  ...
  useEffect(() => {
    ...
    const grouped = allFiles.reduce((acc, file) => {
      const title = file.attributes?.name || 'Unknown';
      if (!acc[title]) acc[title] = { title, artist: file.attributes?.artistName, formats: [] };
      acc[title].formats.push(file);
      return acc;
    }, {});

    const finalGroupedTracks = Object.values(grouped).map(track => {
      track.formats.sort((a, b) => (a.rawMetadata?.formatRank || 100) - (b.rawMetadata?.formatRank || 100));
  ...
  const handleLikeToggle = (songId, song) => {
    ...
    <div className="relative min-h-screen pb-40 overflow-x-hidden font-sans text-white">
      ...
      <div className="relative z-10 px-6 pt-20 md:pt-32 pb-12 max-w-7xl mx-auto flex flex-col gap-8 md:gap-12">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-12 text-center md:text-left">
          <div className="w-[60vw] max-w-[300px] aspect-square md:w-72 md:h-72 shrink-0 shadow-2xl rounded-xl overflow-hidden glass-card flex items-center justify-center bg-white/5 mx-auto md:mx-0">
