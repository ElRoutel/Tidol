...
export default function FullScreenPlayer({ isEmbedded = false }) {
    ...
    const dominantColor = currentSong?.extractedColors?.dominant || '#222222';
    ...
    const dragProps = isEmbedded ? {} : {
        ...
        dragElastic: { top: 0.05, bottom: 0.8 }, // Rebote natural
    ...
    return (
        ...
        >
            {/* 🌌 Background Premium (CSS Radial Gradient for Apple Music Style) */}
            <div 
            ...
            {isDesktop ? (
                ...
                <div className="relative z-10 w-full h-full grid grid-cols-[40%_60%]">
                    ...
                    <div className="fixed bottom-0 left-0 w-full h-[100px] z-50 flex items-center justify-between px-12 bg-gradient-to-t from-black/80 to-transparent backdrop-blur-md">
                        <div className="flex items-center gap-6 w-[30%]">
                            <div className="flex flex-col min-w-0 mr-4">
                                <span className="text-white font-bold text-lg truncate leading-tight tracking-tight">
                ...
                // ================= MOBILE LAYOUT (<1024px) - NIVEL SENIOR =================
                // En el contenedor mobile, reemplaza pb-10 por:
                <div
                    className="relative z-10 flex flex-col h-full w-full mx-auto px-6 pt-10 pb-6"
                >


                    {/* Header Píldora (Cerrar) */}
                    <div className="flex items-center justify-center mb-8 shrink-0 relative z-50">
                        <button
                            onClick={closeFullScreenPlayer}
                            className="w-12 h-1.5 bg-white/20 rounded-full hover:bg-white/40 active:scale-95 transition-all cursor-pointer"
                        />
                    </div>

                    {/* Portada del Álbum (Arte) */}
                    <div className="w-full flex justify-center mb-10 shrink-0 relative">
                        <div
                            className="relative w-[85vw] max-w-[360px] aspect-square shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-transform duration-500 ease-out z-10"
                        ...
                        >
                            ...
                            <img 
                                src={currentSong?.coverArtUrl || currentSong?.portada || currentSong?.coverFull || currentSong?.cover || '/default-album.png'} 
                                alt={currentSong?.trackName || currentSong?.titulo} 
                                className="w-full h-full object-cover" 
                                onError={(e) => { e.currentTarget.src = '/default-album.png'; }}
                            />
                    ...
                    >
                        <div className="relative w-full h-8 flex items-center cursor-pointer">
                            ...
                            <motion.div className="absolute h-3 w-3 bg-white rounded-full shadow-md pointer-events-none transition-transform duration-100" style={{ left: displayWidth, transform: 'translateX(-50%)', willChange: 'left' }} />
                        </div>
                    ...
                    <div className="mt-auto flex flex-col items-center w-full pb-2 relative z-20">
                        <div className="flex items-center justify-between gap-2 mb-10 w-[90%] max-w-[320px]">
                        ...
                        <div className="w-full flex items-center justify-between px-4 text-white/50">
                            ...
                            <button className="hover:text-white transition-colors active:scale-90 p-2"><IoDisc size={24} /></button>
                    ...
                    {viewMode === 'queue' && (
                        <div className="absolute inset-0 z-50 bg-[#050505]/95 backdrop-blur-3xl animate-fade-in pt-16 px-6 touch-auto" onPointerDown={(e) => e.stopPropagation()}>
                            ...
                            <div className="h-full w-full overflow-y-auto no-scrollbar text-white/60 pt-12">
                                    ...
                                    <div className="py-4 space-y-2">
                                        {originalQueue.map((s, idx) => (
                                            <div key={s.id || idx} className={`flex items-center justify-between p-3 rounded-xl transition-all ${idx === currentIndex ? 'bg-white/10' : 'bg-transparent'}`}>
                                                <div className="flex items-center gap-4 min-w-0">
                                                    <button onClick={() => { playAt(idx); setViewMode('cover'); }} className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/20 active:scale-90 shrink-0">
                                                        <IoPlaySharp size={18} />
                                                    </button>
                                                    <div className="min-w-0">
                                                        <div className={`font-semibold truncate text-[16px] ${idx === currentIndex ? 'text-[#1db954]' : 'text-white'}`}>{s.trackName || s.titulo || s.title || 'Untitled'}</div>
            ...
            <AnimatePresence>
                {showOptions && (
                    <motion.div
                        ...
                        className="absolute bottom-24 right-6 z-[100] bg-[#1a1a1a]/90 border border-white/10 rounded-2xl p-2 w-[220px] shadow-[0_20px_40px_rgba(0,0,0,0.5)] backdrop-blur-2xl"
                    >
