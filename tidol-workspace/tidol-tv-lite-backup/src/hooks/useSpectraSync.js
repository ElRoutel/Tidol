...
export function useSpectraSync() {
    const { currentTrack: currentSong, spectraData, updateSpectraData, resetSpectraData, engine } = usePlayer();
    ...
    useEffect(() => {
        ...
        const fetchAnalysis = async () => {
            try {
                ...
                const response = await axios.get(`${SPECTRA_BASE_URL}/analysis${query}`, {
                ...
                try {
                    if (isIA) {
                        ...
                        for (let attempt = 1; attempt <= 2; attempt++) {
                            try {
                                const resp = await axios.post(`${SPECTRA_BASE_URL}/ingest-remote`, {
                                    audioUrl: audioUrlToSend,
                                    metadata: {
