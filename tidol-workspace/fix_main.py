...
def process():
    ...
    import re
    # Remove from `struct DynamicAIProvider {` up to its Drop implementation
    content = re.sub(r'// -{10,}\n// CARGADOR DINÁMICO FFI: MOTOR IA \(CUDA\)\n// -{10,}\nstruct DynamicAIProvider \{.*?\n\}\n', '', content, flags=re.DOTALL)
    
    # 2. Find ai_worker_loop block
    # from `if requiere_ia_completa { // Solo fallback si no hay NADA`
    # up to `            let _ = sqlx::query!(\n                "UPDATE audioProcessingQueue SET status = 'COMPLETED', progress = 100 WHERE taskId = ?",\n                job.taskId\n            )`
...
"""
    ...
    # We use regex to replace the if requiere_ia_completa { ... } block
