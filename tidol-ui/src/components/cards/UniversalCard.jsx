...
const normalizeData = (data) => {
    ...
    // If backend provided a displayTitle (for albums), prefer it
    const isAlbumLike = !!data.displayTitle || data.autor || data.year || data.anio;
    let title = data.displayTitle || data.titulo || data.title || data.name || 'Unknown';

    // Special-case for Internet Archive items: if the track title looks like a numbered track
    // ("01 Foo"), prefer using the album name instead of the first-track title.
    const looksLikeNumberedTrack = typeof data.titulo === 'string' && /^\s*\d{1,2}\b/.test(data.titulo);
    if (data.identifier && data.album) {
        const albumTrim = String(data.album).trim();
        if (looksLikeNumberedTrack) {
            // Use album as display title and make sure it's non-empty
            if (albumTrim.length > 0) title = albumTrim;
        } else if (!isAlbumLike && albumTrim && albumTrim.length > title.length + 5) {
            // If album seems more descriptive than the short track title, prefer it
            title = albumTrim;
        }
    }

    // Subtitle: artist for songs, year/artist for albums
    let subtitle = data.artista || data.artist || data.creator || data.autor || '';
    if (isAlbumLike && data.year) subtitle = data.year.toString();
...
const UniversalCard = React.memo(({
    ...
    const isAlbum = !!item.displayTitle || !!item.year || !!item.autor;
        ...
        return (
            ...
            >
                ...
                <div className="flex flex-col gap-1">
                    <h3 className={`text-sm font-bold text-white truncate ${isAlbum ? 'text-[14px] md:text-[16px]' : ''}`}>{item.title}</h3>
                    {isAlbum ? (
                        <p className="text-xs text-gray-400">{item.year ? item.year : item.subtitle}</p>
                    ) : (
