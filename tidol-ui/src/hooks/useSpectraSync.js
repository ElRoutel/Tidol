...
export function useSpectraSync() {
    ...
    const fetchLyrics = useCallback(async (skipGeneration = false) => {
        ...
        try {
            ...
            if (!skipGeneration) {
                try {
                    if (isIA) {
                        ...
                        const audioUrlToSend = currentSong.originalUrl || currentSong.url || '';
                        ...
                        for (let attempt = 1; attempt <= 2; attempt++) {
                            try {
                                const resp = await fetch(`${SPECTRA_BASE_URL}/ingest-remote`, {
                                    method: 'POST',
            ...
            if (!lrcText) throw new Error('Empty lyrics received');
            ...
            if (error && error.isNotFound) {
                console.log('[Spectra] Lyrics are still being processed, status set to loading');
                updateSpectraData({ status: 'loading' });
                throw error;
            }
